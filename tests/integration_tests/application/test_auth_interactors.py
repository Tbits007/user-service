...
from app.application.interactors.auth_interactors import LoginInteractor, PasswordResetConfirmInteractor, PasswordResetInteractor, RegisterInteractor, VerifyInteractor
...
@pytest.mark.asyncio
async def test_register_interactor(mock_user_saver, mock_email_sender, mock_jwt_processor, mock_password_hasher):
    # Подготовка
    # Входные данные
...
async def test_verify_interactor(mock_user_updater, mock_jwt_processor):
    ...
    # Проверки
    mock_jwt_processor.verify_token.assert_called_once_with(token, token_type=TokenType.ACCESS)
    mock_user_updater.update.assert_called_once_with(email, {"is_verified": True})


@pytest.mark.asyncio
async def test_login_interactor(mock_user_reader, mock_password_hasher, mock_jwt_processor):
    # Подготовка
    dto = LoginUserDTO(email="test@example.com", password="password123")
    user = UserDM(email=dto.email, username="TestUser", password="hashed_password", is_verified=True)
    ...
    mock_password_hasher.verify_password.assert_called_once_with(dto.password, user.password)
    assert tokens == {"access_token": "access_token", "refresh_token": "refresh_token"}


@pytest.mark.asyncio
async def test_password_reset_interactor(mock_user_reader, mock_email_sender, mock_jwt_processor):
    # Подготовка
    email = "test@example.com"
...
@pytest.mark.asyncio
async def test_password_reset_confirm_interactor(mock_user_updater, mock_password_hasher, mock_jwt_processor):
    # Подготовка
    token = "reset_token"
    ...
    # Проверки
    mock_jwt_processor.verify_token.assert_called_once_with(token, token_type=TokenType.PASSWORD_RESET)
    mock_password_hasher.get_password_hash.assert_called_once_with(new_password)
    mock_user_updater.update.assert_called_once_with(email, {"password": hashed_password})
